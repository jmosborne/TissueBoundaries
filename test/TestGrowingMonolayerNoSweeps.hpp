...
static const unsigned M_GHOST_LAYER_THICKNESS = 2;
...
private:
    ...
    */
    void RoundOutCellPopulation(AbstractCellPopulation<2>& rCellPopulation)
    {
        ...
        {
            MutableMesh<2, 2>& r_mesh = static_cast<MeshBasedCellPopulationWithGhostNodes<2>* >(&rCellPopulation)->rGetMesh();
                ...
                cell_iter != rCellPopulation.End();)
            {
                ...
                // if ((fabs(y-x)>M_DOMAINWIDTH) && (x<M_DOMAIN_X_MIN) && (x>M_DOMAIN_X_MAX) && (y<M_DOMAIN_Y_MIN) && (y>M_DOMAIN_Y_MAX))
                if ( norm_2(centre_of_cell) > (M_DOMAIN_WIDTH + double(M_GHOST_LAYER_THICKNESS) ) )
                {   
                    // Delete ghost nodes outside of region
                    cell_iter->Kill();
                }
            }
...
public:
    ...
    void TestMeshBasedGhostsGrowingMonolayer()
    {
        ...
        HoneycombMeshGenerator generator(2.0*M_INITIAL_WIDTH, 3.0*M_INITIAL_LENGTH, M_GHOST_LAYER_THICKNESS);
        ...
        // Add ghost removal modifier
        MAKE_PTR(GhostNodeCollisionRemovalModifier<2>, p_modifier_2);
